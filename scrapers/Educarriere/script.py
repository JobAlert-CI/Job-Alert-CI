...
from extract_offer import parse_job_html_educ
...
def get_data_job(html: HTMLParser, url: str) -> dict[str, Any]:
    ...
    """
    logger.info(f"Début de l'extraction des données sur {url} via extract_offer_Educ.")
    
    # 1. Appel au nouveau parser centralisé
    job_data = parse_job_html_educ(html, source_url=url)
    
    # 2. Couche d'adaptation (Adapter) pour la rétrocompatibilité
    # (Au cas où ton API ou la suite du script attend les anciennes clés)
        
    # Mapping de la spécialité vers l'ancienne clé "secteur"
    if job_data.get("specialty") and not job_data.get("secteur"):
        job_data["secteur"] = job_data["specialty"]
        
    # Reconstruction du dictionnaire "company" pour l'API
    company_data = {
        "name": job_data.get("company_name"),
        "website_url": job_data.get("company_website"),
        "type": job_data.get("company_type"),
