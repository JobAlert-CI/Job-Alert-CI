...
export const DialogEdition = ({ open, admin, moiMeme, mutation, onFermer }) => {
  ...
  /* Synchronisation depuis l'admin ciblé à chaque ouverture. */
  useEffect(() => {
    if (!open || !admin) return
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setEmail(admin.email)
    setNom(admin.full_name)
    setActif(admin.is_active)
    setErreur(null)
  }, [open, admin])
      ...
      {admin && (
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            ...
            {/* type="button" + onClick : le bouton est HORS du <form>
                (DialogFooter) ; la touche Entrée passe elle par le
                onSubmit du form. Un seul chemin par déclencheur. */}
            <Button
              type="button"
...
export const DialogRole = ({ open, admin, moiMeme, mutation, onFermer }) => {
  const notify = useNotify()
  const [role, setRole] = useState("")
  const [erreur, setErreur] = useState(null)

  /* Synchronisation sur le rôle actuel de l'admin ciblé à l'ouverture. */
  useEffect(() => {
    if (!open || !admin) return
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setRole(admin.role)
...
export const DialogSuppression = ({ open, admin, mutation, onFermer }) => {
  const notify = useNotify()

  const supprimer = () =>
    admin && mutation.mutate(admin.id, {
      onSuccess: () => {
